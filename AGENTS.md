...
- `.github/workflows/python-publish.yml` (workflow name Release) publishes on `workflow_dispatch` bump or a `vX.Y.Z` tag push via OIDC trusted publishing. Filename must stay `python-publish.yml` to match the PyPI trusted publisher. Unprefixed tags like `0.1.7` no longer trigger publish. Do not auto-publish on every push to main.
...
- README images in `assets/` must use absolute `https://raw.githubusercontent.com/SSujitX/google-news-url-decoder/main/assets/...` URLs so they render on PyPI; relative paths work on GitHub only.
