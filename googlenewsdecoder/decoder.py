...
class GoogleDecoder:
    """Synchronous decoder. Use as a context manager so the HTTP client is closed.
    ...
    def __enter__(self) -> GoogleDecoder:
        return self
    ...
    def get_decoding_params(self, art_id: str, source_url: str | None = None) -> dict:
    ...
    def _batchexecute(self, items: list[tuple[str, str, str]]) -> dict:
    ...
    def decode_google_news_url(
        ...
        """
        results = self.decode_google_news_urls([source_url], interval=interval)
        return results[0]

    def decode_google_news_urls(
        self, source_urls: list[str], interval: int | None = None
    ) -> list[dict]:
        """Decode many URLs. Article pages are fetched in order, then one POST.
        ...
        for i, source_url in enumerate(source_urls):
            ...
            if not art_id:
                ...
                continue
            params = self.get_decoding_params(art_id, source_url)
